...
try({
      ...
      output_width <- 15
      output_height <- 15
      for (i in c(seq(1, total_epochs, 4), total_epochs)) {
        for (cur_key in keys_to_plot) {
          ...
          plt <- ggplot(cur_emb) +
            geom_point(aes(x = main.1, y = main.2, color = data_key), alpha = 0.5) +
            scale_color_brewer(palette = 'Paired') +
            theme_classic() +
            theme(legend.position = 'bottom')
          plt <- ggMarginal(plt, groupColour = TRUE, groupFill = TRUE)
          plt_path <- './runs/EX07_08/plots/main_lat'
...
try({
      ...
      for (i in c(seq(1, total_epochs, 4), total_epochs)) {
        for (cur_key in keys_to_plot) {
          ...
          plt_path <- './runs/EX07_08/plots/pheno_distrib'
          system(paste0('mkdir -p ', plt_path))
