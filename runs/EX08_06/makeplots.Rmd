...
try({
      ...
      for (i in sort(c(seq(5, total_epochs, 15), seq(15, total_epochs, 15)))) {
        for (cur_key in keys_to_plot) {
          ...
          plt_path <- './runs/EX08_06/plots/main_lat'
...
try({
      ...
      # for (i in c(seq(1, total_epochs, 9), total_epochs)) {
      for (i in sort(1 + c(seq(5, total_epochs, 15), seq(15, total_epochs, 15)))) {
        for (cur_key in keys_to_plot) {
          emb_filename <- paste0(emb_path, '/', i, '_', cur_key, '.csv')
          cur_emb <- read.csv(emb_filename) %>%
            left_join(y = pheno_df %>% select(cell, data_key), by = 'cell')
          plt <- ggplot(cur_emb) +
            ...
            theme(legend.position = 'bottom')
          plt <- ggMarginal(plt, groupColour = TRUE, groupFill = TRUE)
          plt_path <- './runs/EX08_06/plots/data_key_distrib'
          system(paste0('mkdir -p ', plt_path))
      ...
      for (i in sort(1 + c(seq(5, total_epochs, 15), seq(15, total_epochs, 15)))) {
        for (cur_key in keys_to_plot) {
          ...
          plt <- ggplot(cur_emb) +
            ...
            theme(legend.position = 'bottom')
          plt <- ggMarginal(plt, groupColour = TRUE, groupFill = TRUE)
          plt_path <- './runs/EX08_06/plots/protein_markers_distrib'
          system(paste0('mkdir -p ', plt_path))
