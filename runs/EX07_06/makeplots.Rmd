...
try({
      ...
      for (i in c(seq(1, total_epochs, 9), total_epochs)) {
        for (cur_key in keys_to_plot) {
          ...
          plt_path <- './runs/EX07_06/plots/main_lat'
...
try({
      ...
      for (i in c(seq(1, total_epochs, 9), total_epochs)) {
        for (cur_key in keys_to_plot) {
          ...
          plt <- ggplot(cur_emb) +
            geom_point(aes(x = data_key.1, y = data_key.2, color = data_key)) +
            theme_classic() +
            theme(legend.position = 'bottom')
          plt <- ggMarginal(plt, groupColour = TRUE, groupFill = TRUE)
          plt_path <- './runs/EX07_06/plots/pheno_lat'
          system(paste0('mkdir -p ', plt_path))
...
try({
      ...
      for (i in c(seq(1, total_epochs, 9), total_epochs)) {
        for (cur_key in keys_to_plot) {
          ...
          plt_path <- paste0(getwd(), '/runs/EX07_06/plots/3d_plt/', i, '_main.1_main.2_data_key_', cur_key)
          system(paste0('mkdir -p ', plt_path))
...
```
/Users/hahaschool/Desktop/Research/SAKRA/venv/bin/python "/Volumes/Macintosh HD/Applications/PyCharm.app/Contents/plugins/python/helpers/pydev/pydevconsole.py" --mode=client --port=58058
import sys; print('Python %s on %s' % (sys.version, sys.platform))
sys.path.extend(['/Users/hahaschool/Desktop/Research/SAKRA'])
PyDev console: starting.
Python 3.7.8 (default, Jul  4 2020, 10:17:17)
[Clang 11.0.3 (clang-1103.0.32.62)] on darwin
>>> runfile('/Users/hahaschool/Desktop/Research/SAKRA/SAKRA.py', args=['--config', './runs/EX07_06/config.json', '--verbose', 'True'], wdir='/Users/hahaschool/Desktop/Research/SAKRA')
...
Reading phenotype metadata json from: ./runs/EX07_06/pheno_meta.json
==========================
