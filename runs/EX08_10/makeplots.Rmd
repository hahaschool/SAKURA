...
try({
        ...
        for (i in c(8, 17, 26, 36, 45, 54, 73, 82, 92, 101, 110, 120, 129, 138, 147, 156, 165, 175, 184, 193, 203, 212, 221, 230)) {
            for (cur_key in keys_to_plot) {
                ...
                plt_path <- './runs/EX08_10/plots/main_lat'
        ...
        for (i in c(8, 17, 26, 36, 45, 54, 73, 82, 92, 101, 110, 120, 129, 138, 147, 156, 165, 175, 184, 193, 203, 212, 221, 230)) {
            for (cur_key in keys_to_plot) {
                ...
                plt <- ggplot(cur_emb) +
                    ...
                    theme(legend.position = 'bottom')
                plt <- ggMarginal(plt, groupColour = TRUE, groupFill = TRUE)
                plt_path <- './runs/EX08_10/plots/data_key_distrib'
                system(paste0('mkdir -p ', plt_path))
        ...
        for (i in c(8, 17, 26, 36, 45, 54, 73, 82, 92, 101, 110, 120, 129, 138, 147, 156, 165, 175, 184, 193, 203, 212, 221, 230)) {
            for (cur_key in keys_to_plot) {
                ...
                plt <- ggplot(cur_emb) +
                    ...
                    theme_classic() +
                    theme(legend.position = 'bottom')
                plt_path <- './runs/EX08_10/plots/source_distrib'
                system(paste0('mkdir -p ', plt_path))
        ...
        for (i in c(8, 17, 26, 36, 45, 54, 73, 82, 92, 101, 110, 120, 129, 138, 147, 156, 165, 175, 184, 193, 203, 212, 221, 230)) {
            for (cur_key in keys_to_plot) {
                ...
                plt <- ggplot(cur_emb) +
                    ...
                    theme(legend.position = 'bottom')
                plt <- ggMarginal(plt, groupColour = TRUE, groupFill = TRUE)
                plt_path <- './runs/EX08_10/plots/protein_markers_distrib'
                system(paste0('mkdir -p ', plt_path))
...
try({
        ...
        for (i in c(seq(1, total_epochs, 9), total_epochs)) {
            for (cur_key in keys_to_plot) {
                ...
                plt_path <- paste0(getwd(), '/runs/EX08_10/plots/3d_plt/', i, '_main.1_main.2_data_key_', cur_key)
                system(paste0('mkdir -p ', plt_path))
