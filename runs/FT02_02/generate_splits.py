...
pheno_csv_path = '../../dataset/10X_PBMC_V1_V3_V31_integrated/downsampled/downsampled_pheno.csv'
...
splits['protein_v3'] = np.array(pheno_df.source == 'V3_with_protein_barcode', dtype=np.int)
splits['pure_v1'] = np.array(pheno_df.source == 'v1_purified', dtype=np.int)
splits['targeted_v31'] = np.array(pheno_df.source == 'V31_targeted_immune_panel', dtype=np.int)
...
# V31 Targeted: train-test
v3_overall_train_dec = 8
v3_overall_train_seed = 1919

# Make v31 train/test split
v3_dec_bin = data_splitter.auto_random_k_bin_labelling(base=splits['targeted_v31'],
                                                       k=10,
                                                       seed=v3_overall_train_seed)
overall_train_test_split = data_splitter.get_incremental_train_test_split(base=v3_dec_bin,
                                                                          k=v3_overall_train_dec)
splits['targeted_v31_overall_train'] = overall_train_test_split['train'].astype(np.bool)
splits['targeted_v31_overall_test'] = overall_train_test_split['test'].astype(np.bool)

# Make merged overall train/test split
splits['merged_overall_train'] = splits['pure_v1_overall_train'] | splits['protein_v3_overall_train'] | splits['targeted_v31_overall_train']
splits['merged_overall_test'] = splits['pure_v1_overall_test'] | splits['protein_v3_overall_test'] | splits['targeted_v31_overall_test']
