...
```{r paths}
emb_path <- './runs/EX08_09/logs'
pheno_df_path <- "./dataset/10X_PBMC_integrated_purified_and_cite/pheno_with_source.csv"
...
try({
        ...
        for (i in c(17, 36, 54, 73, 92, 110, 129, 147, 165, 184, 203, 221, 240, 258, 277, 295, 313, 332, 351, 369, 407, 425, 443, 461)) {
            for (cur_key in keys_to_plot) {
                ...
                plt_path <- './runs/EX08_09/plots/main_lat'
        ...
        for (i in c(17, 36, 54, 73, 92, 110, 129, 147, 165, 184, 203, 221, 240, 258, 277, 295, 313, 332, 351, 369, 407, 425, 443, 461)) {
            for (cur_key in keys_to_plot) {
                ...
                plt <- ggplot(cur_emb) +
                    ...
                    theme(legend.position = 'bottom')
                plt <- ggMarginal(plt, groupColour = TRUE, groupFill = TRUE)
                plt_path <- './runs/EX08_09/plots/data_key_distrib'
                system(paste0('mkdir -p ', plt_path))
        ...
        for (i in c(17, 36, 54, 73, 92, 110, 129, 147, 165, 184, 203, 221, 240, 258, 277, 295, 313, 332, 351, 369, 407, 425, 443, 461)) {
            for (cur_key in keys_to_plot) {
                ...
                plt <- ggplot(cur_emb) +
                    ...
                    theme_classic() +
                    theme(legend.position = 'bottom')
                plt_path <- './runs/EX08_09/plots/source_distrib'
                system(paste0('mkdir -p ', plt_path))
        ...
        for (i in c(17, 36, 54, 73, 92, 110, 129, 147, 165, 184, 203, 221, 240, 258, 277, 295, 313, 332, 351, 369, 407, 425, 443, 461)) {
            for (cur_key in keys_to_plot) {
                ...
                plt <- ggplot(cur_emb) +
                    ...
                    theme(legend.position = 'bottom')
                plt <- ggMarginal(plt, groupColour = TRUE, groupFill = TRUE)
                plt_path <- './runs/EX08_09/plots/protein_markers_distrib'
                system(paste0('mkdir -p ', plt_path))
...
try({
        ...
        for (i in c(seq(1, total_epochs, 9), total_epochs)) {
            for (cur_key in keys_to_plot) {
                ...
                plt_path <- paste0(getwd(), '/runs/EX08_09/plots/3d_plt/', i, '_main.1_main.2_data_key_', cur_key)
                system(paste0('mkdir -p ', plt_path))
