...
try({
        ...
        for (i in c(28, 59, 89, 120, 151, 181, 212, 242, 272, 303, 333, 363)) {
            for (cur_key in keys_to_plot) {
                ...
                plt_path <- './runs/EX09_02/plots/main_lat'
        ...
        for (i in c(28, 59, 89, 120, 151, 181, 212, 242, 272, 303, 333, 363)) {
            for (cur_key in keys_to_plot) {
                ...
                plt <- ggplot(cur_emb) +
                    ...
                    theme(legend.position = 'bottom')
                plt <- ggMarginal(plt, groupColour = TRUE, groupFill = TRUE)
                plt_path <- './runs/EX09_02/plots/data_key_distrib'
                system(paste0('mkdir -p ', plt_path))
        ...
        for (i in  c(28, 59, 89, 120, 151, 181, 212, 242, 272, 303, 333, 363)) {
            for (cur_key in keys_to_plot) {
                ...
                plt <- ggplot(cur_emb) +
                    ...
                    theme_classic() +
                    theme(legend.position = 'bottom')
                plt_path <- './runs/EX09_02/plots/source_distrib'
                system(paste0('mkdir -p ', plt_path))
        ...
        for (i in c(28, 59, 89, 120, 151, 181, 212, 242, 272, 303, 333, 363)) {
            for (cur_key in keys_to_plot) {
                ...
                plt <- ggplot(cur_emb) +
                    ...
                    theme(legend.position = 'bottom')
                plt <- ggMarginal(plt, groupColour = TRUE, groupFill = TRUE)
                plt_path <- './runs/EX09_02/plots/protein_markers_distrib'
                system(paste0('mkdir -p ', plt_path))
        ...
        for (i in c(333, 363)) {
            for (key_idx in 1:length(keys_to_plot)) {
                ...
                for (cur_pure in data_key_pure_to_inspect) {
                    ...
                    plt_path <- './runs/EX09_02/plots/data_key_vs_protein_markers'
                    system(paste0('mkdir -p ', plt_path))
            ...
            for (key_idx in 1:length(keys_to_plot)) {
                ...
                for (cur_pure in data_key_pure_to_inspect) {
                    ...
                    plt <- ggplot() +
                        ...
                        scale_alpha_continuous(range = c(0, 1)) +
                        facet_wrap(vars(variable))
                    plt_path <- './runs/EX09_02/plots/protein_markers_vs_protein_markers'
                    system(paste0('mkdir -p ', plt_path))
                    plt_filename <- paste0(plt_path, '/', i, '_', cur_key, '_', cur_pure, '.jpg')
                    ggsave(plt, filename = plt_filename, width = output_width, height = output_height)
            ...
            for (key_idx in 1:length(keys_to_plot)) {
                ...
                for (cur_pure in data_key_pure_to_inspect) {
                    ...
                    plt <- ggplot() +
                        ...
                        scale_alpha_continuous(range = c(0, 1)) +
                        facet_wrap(vars(variable))
                    plt_path <- './runs/EX09_02/plots/main_vs_protein_markers'
                    system(paste0('mkdir -p ', plt_path))
                    plt_filename <- paste0(plt_path, '/', i, '_', cur_key, '_', cur_pure, '.jpg')
                    ggsave(plt, filename = plt_filename, width = output_width, height = output_height)
...
try({
        ...
        for (i in c(333, 363)) {
            ...
            plt_path <- './runs/EX09_02/plots/cell_mixture'
            system(paste0('mkdir -p ', plt_path))
...
try({
        ...
        for (i in c(seq(1, total_epochs, 9), total_epochs)) {
            for (cur_key in keys_to_plot) {
                ...
                plt_path <- paste0(getwd(), '/runs/EX09_02/plots/3d_plt/', i, '_main.1_main.2_data_key_', cur_key)
                system(paste0('mkdir -p ', plt_path))
