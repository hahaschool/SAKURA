...
try({
      ...
      for (i in 30:total_epochs) {
        for (cur_key in keys_to_plot) {
          ...
          plt_path <- './runs/EX05_04/plots/signature_lat'
...
try({
      ...
      for (i in 30:total_epochs) {
        for (cur_key in keys_to_plot) {
          ...
          plt_path <- './runs/EX05_04/plots/signature_distrib'
          system(paste0('mkdir -p ', plt_path))
  ...
  for (cur_key in keys_to_plot[2]) {
    for (cur_signature in signatures_to_plot) {
      ...
      plt <- ggplot(cur_emb) +
              ...
              geom_density2d(data = cur_emb %>% filter(value > 0), aes(x = sgn_score, y = value)) +
