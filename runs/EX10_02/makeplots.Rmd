...
try({
        ...
        for (i in c(19, 41, 63, 85, 107, 128, 151, 173, 193, 216)) {
            for (cur_key in keys_to_plot) {
                ...
                plt_path <- './runs/EX10_02/plots/main_lat'
        ...
        for (i in c(19, 41, 63, 85, 107, 128, 151, 173, 193, 216)) {
            for (cur_key in keys_to_plot) {
                ...
                plt <- ggplot(cur_emb) +
                    ...
                    theme(legend.position = 'bottom')
                plt <- ggMarginal(plt, groupColour = TRUE, groupFill = TRUE)
                plt_path <- './runs/EX10_02/plots/data_key_distrib'
                system(paste0('mkdir -p ', plt_path))
        ...
        for (i in  c(19, 41, 63, 85, 107, 128, 151, 173, 193, 216)) {
            for (cur_key in keys_to_plot) {
                ...
                plt <- ggplot(cur_emb) +
                    ...
                    theme_classic() +
                    theme(legend.position = 'bottom')
                plt_path <- './runs/EX10_02/plots/source_distrib'
                system(paste0('mkdir -p ', plt_path))
        ...
        for (i in  c(19, 41, 63, 85, 107, 128, 151, 173, 193, 216)) {
            for (cur_key in keys_to_plot) {
                ...
                plt <- ggplot(cur_emb) +
                    ...
                    theme(legend.position = 'bottom')
                plt <- ggMarginal(plt, groupColour = TRUE, groupFill = TRUE)
                plt_path <- './runs/EX10_02/plots/outside_panel'
                system(paste0('mkdir -p ', plt_path))
        ...
        for (i in c(19, 41, 63, 85, 107, 128, 151, 173, 193, 216)) {
            for (cur_key in keys_to_plot) {
                ...
                plt <- ggplot(cur_emb) +
                    ...
                    theme(legend.position = 'bottom')
                plt <- ggMarginal(plt, groupColour = TRUE, groupFill = TRUE)
                plt_path <- './runs/EX10_02/plots/protein_markers_distrib'
                system(paste0('mkdir -p ', plt_path))
        ...
        for (i in c(19, 128, 216)) {
            for (key_idx in 1:length(keys_to_plot)) {
                ...
                for (cur_pure in data_key_pure_to_inspect) {
                    ...
                    molten_emb <- cur_emb %>%
                        melt(id.vars = c("cell", "main.1", "main.2", "data_key.1", "data_key.2", "protein_markers.1", "protein_markers.2", "source.1", "data_key", "source", "SingleR_immuneDB_main", "SingleR_immuneDB_fine", "SingleR_monaco_main", "SingleR_monaco_fine", "outside_panel.1", "outside_panel.2"))
                    ...
                    plt_path <- './runs/EX10_02/plots/data_key_vs_protein_markers'
                    system(paste0('mkdir -p ', plt_path))
            ...
            for (key_idx in 1:length(keys_to_plot)) {
                ...
                for (cur_pure in data_key_pure_to_inspect) {
                    ...
                    plt <- ggplot() +
                        ...
                        scale_alpha_continuous(range = c(0, 1)) +
                        facet_wrap(vars(variable))
                    plt_path <- './runs/EX10_02/plots/protein_markers_vs_protein_markers'
                    system(paste0('mkdir -p ', plt_path))
                    plt_filename <- paste0(plt_path, '/', i, '_', cur_key, '_', cur_pure, '.jpg')
                    ggsave(plt, filename = plt_filename, width = output_width, height = output_height)
            ...
            for (key_idx in 1:length(keys_to_plot)) {
                ...
                for (cur_pure in data_key_pure_to_inspect) {
                    ...
                    plt <- ggplot() +
                        ...
                        scale_alpha_continuous(range = c(0, 1)) +
                        facet_wrap(vars(variable))
                    plt_path <- './runs/EX10_02/plots/main_vs_protein_markers'
                    system(paste0('mkdir -p ', plt_path))
                    plt_filename <- paste0(plt_path, '/', i, '_', cur_key, '_', cur_pure, '.jpg')
                    ggsave(plt, filename = plt_filename, width = output_width, height = output_height)
...
try({
        ...
        for (i in c(19, 128, 216)) {
            ...
            plt_path <- './runs/EX10_02/plots/cell_mixture'
            system(paste0('mkdir -p ', plt_path))
...
try({
        ...
        for (i in c(193, 216)) {
            for (cur_key in keys_to_plot) {
                ...
                plt_path <- paste0(getwd(), '/runs/EX10_02/plots/3d_plt/', i, '_protein_markers_source_', cur_key)
                system(paste0('mkdir -p ', plt_path))
